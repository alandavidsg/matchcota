...
export default function ReportarPage() {
  ...
  const handleAdoptSubmit = async () => {
    ...
    setSubmitted(true);
    // La confirmación flotante se muestra al llegar a la ficha: mostrarla acá la
    // cortaba esta misma redirección, 1,5s después.
    setTimeout(() => router.push(nueva?.id ? `/mascota/${nueva.id}?publicada=adopcion` : '/'), 1500);
  };
  ...
  const publishNew = async () => {
    ...
    setSubmitted(true);
    setTimeout(() => router.push(nueva?.id ? `/mascota/${nueva.id}?publicada=calle` : '/'), 1500);
  };
  ...
  const linkToExisting = async (mascotaId: number) => {
    ...
    setShowDupModal(false);
    setSubmitted(true);
    setTimeout(() => router.push(`/mascota/${mascotaId}?publicada=avistamiento`), 1500);
  };
